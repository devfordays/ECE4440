if { ![project_exists {Fetch_Stage}] } {
     project_new {Fetch_Stage}
} else {
     project_open {Fetch_Stage}
}
set_project_settings -cmp {Fetch_Stage}
if { ![project_settings_exist -sim Fetch_Stage] } {
        set_project_settings -sim Fetch_Stage
}
set_global_assignment -section_id {Fetch_Stage} -name {EDA_DESIGN_ENTRY_SYNTHESIS_TOOL} {PRECISION SYNTHESIS}
set_global_assignment -name {VQM_FILE} {Fetch_Stage.vqm}
...
if { [file exists {Fetch_Stage_altera.prop}] } {
  source Fetch_Stage_altera.prop
}
set_global_assignment -name {FAMILY} {CYCLONE II}
set_global_assignment -section_id {Fetch_Stage} -name {DEVICE} {EP2C70F896I8}
set_global_assignment -name {PROJECT_SHOW_ENTITY_NAME} {OFF}
set_global_assignment -name {SDC_FILE} {Fetch_Stage_pnr_constraints.sdc}
