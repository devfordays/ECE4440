if { ![project_exists {Decode_Stage}] } {
     project_new {Decode_Stage}
} else {
     project_open {Decode_Stage}
}
set_project_settings -cmp {Decode_Stage}
if { ![project_settings_exist -sim Decode_Stage] } {
        set_project_settings -sim Decode_Stage
}
set_global_assignment -section_id {Decode_Stage} -name {EDA_DESIGN_ENTRY_SYNTHESIS_TOOL} {PRECISION SYNTHESIS}
set_global_assignment -name {VQM_FILE} {Decode_Stage.vqm}
...
if { [file exists {Decode_Stage_altera.prop}] } {
  source Decode_Stage_altera.prop
}
set_global_assignment -name {FAMILY} {CYCLONE II}
set_global_assignment -section_id {Decode_Stage} -name {DEVICE} {EP2C70F896I8}
set_global_assignment -name {PROJECT_SHOW_ENTITY_NAME} {OFF}
set_global_assignment -name {SDC_FILE} {Decode_Stage_pnr_constraints.sdc}
